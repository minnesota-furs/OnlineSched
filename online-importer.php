...
function handle_event_schedule_csv_upload($file) {
    if (($handle = fopen($file['tmp_name'], 'r')) !== FALSE) {
        ...
        $required_headers = array('name', 'DateTime', 'description', 'room_type', 'Speakers', 'length');
        $header_indexes = array();

        foreach ($required_headers as $required_header) {
            $index = array_search(strtolower($required_header), array_map('strtolower', $headers));
            if ($index === false) {
                echo '<div class="error"><p>CSV file is missing required header: ' . $required_header . '</p></div>';
                return;
            }
            $header_indexes[$required_header] = $index;
        }
        ...
        while (($data = fgetcsv($handle, 4000, ',')) !== FALSE) {
            ...
            if (count($data) < count($required_headers)) {
                echo "<div class='error'><p>Row $row has fewer fields than expected. Please ensure all required fields are present.</p></div>";
                continue;
            }

            $name = sanitize_text_field($data[$header_indexes['name']]);
            $datetime = sanitize_text_field($data[$header_indexes['DateTime']]);
            $description = sanitize_text_field($data[$header_indexes['description']]);
            $room_type = sanitize_text_field($data[$header_indexes['room_type']]);
            $speakers = sanitize_text_field($data[$header_indexes['Speakers']]);
            $length = sanitize_text_field($data[$header_indexes['length']]);
            ...
            wp_set_post_terms($event_id, array(), 'event_schedule_panelist_type');
            ...
            if ($day_of_week !== 'Unscheduled') {
                $event_schedule_day_type_term = wp_insert_term($day_of_week, 'event_schedule_day_type', array(
                    'description' => $formatted_date
                ));

                if (!is_wp_error($event_schedule_day_type_term)) {
                    wp_set_post_terms($event_id, array($event_schedule_day_type_term['term_id']), 'event_schedule_day_type');
                }
            } else {
            ...
            wp_set_post_terms($event_id, $panelist_terms, 'event_schedule_panelist_type');
        }
        fclose($handle);
...
function export_event_schedule_csv() {
    ...
    fputcsv($output, array('name', 'DateTime', 'description', 'room_type', 'Speakers', 'length'));
