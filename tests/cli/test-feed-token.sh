...
CONTAINER="${ONLINESCHED_FEED_TEST_CONTAINER:-onlinesched-vanilla-cli}"
WP=(docker exec "$CONTAINER" wp --allow-root --path=/var/www/html)
CONTAINER_TEST="/var/www/html/wp-content/plugins/OnlineSched/tests/cli/test-feed-token.php"

if [[ "$CONTAINER" != "onlinesched-vanilla-cli" ]]; then
	echo "Refusing unrecognized feed test container: $CONTAINER" >&2
	exit 1
fi
