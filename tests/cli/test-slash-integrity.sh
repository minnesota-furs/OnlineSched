...
CONTAINER_TEST="/var/www/html/wp-content/plugins/OnlineSched/tests/cli/test-slash-integrity.php"
BASE_URL="http://localhost:8081"
START_HINT="cd tests/docker-vanilla && docker compose up -d && ./seed-vanilla.sh"

if [[ "$CONTAINER" != "onlinesched-vanilla-cli" ]]; then
	echo "Refusing unrecognized slash integrity test container: $CONTAINER" >&2
	exit 1
fi
